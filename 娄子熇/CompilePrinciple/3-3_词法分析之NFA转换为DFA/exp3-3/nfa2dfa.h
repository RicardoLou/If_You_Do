...
struct DFA            /*定义DFA结构*/
{
    ...
    int trans[MAX][36];        /*DFA的转移矩阵，支持26个字母和10个数字*/
};
...
DFA nfaToDfa(NFA n, string str)        /*参数为nfa和后缀表达式*/
{
    ...
    for (i = 0; i <strLength; i++)            /*遍历后缀表达式*/
    {
        if ((str[i] >= 'a' && str[i] <= 'z') || (str[i] >= '0' && str[i] <= '9'))        /*如果遇到操作数，则把它加入到dfa的终结符集中*/
        {
        ...
        for (it = d.terminator.begin(); it != d.terminator.end(); it++)        /*遍历终结符集*/
        {
            ...
            if (!states.count(temp) && !temp.empty())    /*如果求出来的状态集不为空且与之前求出来的状态集不同，则新建一个DFA状态*/
            {
                ...
                if (isAlpha(*it)) {
                    ...
                    d.trans[num][*it - '0' + 26] = dfaStateNum;        /*更新转移矩阵*/
                }
...
void printDFA(DFA d) {
    ...
    for (i = 0; i < dfaStateNum; i++) {
        ...
        for (j = 0; j < 36; j++) {
            if (j < 26 && d.terminator.count(j + 'a')) {
                if (d.trans[i][j] != -1) {
                    cout << d.trans[i][j] << "   ";
                } else {
                    cout << "    ";
                }
            } else if (j >= 26 && d.terminator.count(j - 26 + '0')) {
                if (d.trans[i][j] != -1) {
                    cout << d.trans[i][j] << "   ";
                } else {
                    cout << "    ";
